    ...
    <section id="about" className="py-20 bg-white w-full">
      <div className="w-full px-6 md:px-12 lg:px-20">
        ...
        <div className="text-center mb-16 animate-fade-in-up">
          <h2 className="text-5xl md:text-6xl font-cinzel text-gray-900 mb-4 tracking-wide">
            About{' '}
            <span className="font-cinzel font-bold text-yellow-400 text-6xl md:text-7xl tracking-tight">
              M&F Kitchen
            </span>
          </h2>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center mb-24 animate-fade-in-up">
          ...
          <div className="w-full h-full">
            <video
              ...
              muted
              playsInline
              className="rounded-3xl shadow-xl w-full h-[500px] object-cover transition-transform duration-500 transform hover:scale-105"
          ...
          <div className="space-y-6 text-xl text-gray-700 leading-relaxed">
            ...
            <div className="flex items-center space-x-4 pt-6">
              <img
                src="/logo.png"
                alt="M&F Logo"
                className="w-20 h-20 rounded-full border border-primary object-cover shadow"
              />
              <div>
                <p className="font-cinzel text-2xl font-semibold text-gray-800">
                  M&F Kitchen
                </p>
                <p className="text-gray-600 text-lg">Ghar Jaisa Khana. Cloud Mein.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Stats Section */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-24 text-center animate-slide-up">
          {[
            { icon: <Users className="h-14 w-14 text-primary mx-auto mb-3" />, title: '500+', subtitle: 'Happy Customers' },
            { icon: <Heart className="h-14 w-14 text-primary mx-auto mb-3" />, title: '100%', subtitle: 'Fresh Ingredients' },
            { icon: <Utensils className="h-14 w-14 text-primary mx-auto mb-3" />, title: 'Daily', subtitle: 'Fresh Cooking' },
            {
              icon: <Clock className="h-14 w-14 text-primary mx-auto mb-3" />,
              title: isOpen ? ' Open Now' : ' Closed',
              subtitle: 'Mon–Sat: 10:00 AM – 10:00 PM'
            }
          ].map((item, index) => (
            <div key={index} className="bg-yellow-50 p-10 rounded-2xl shadow hover:shadow-xl transition">
              {item.icon}
              <h4 className={`text-3xl font-bold ${isOpen ? 'text-green-600' : 'text-red-500'} mb-1`}>
                {item.title}
              </h4>
        ...
        <div className="text-center w-full animate-fade-in-up">
          <h3 className="text-4xl md:text-5xl font-semibold text-gray-800 mb-6">Why Choose Us?</h3>
          ...
          <div className="grid md:grid-cols-3 gap-10">
            {[
              {
                icon: <Star className="h-10 w-10 text-primary mx-auto mb-3" />,
                title: 'Tastes Like Home',
                desc: 'Every recipe is tried, tested, and rooted in family tradition.',
              },
              {
                icon: <Heart className="h-10 w-10 text-primary mx-auto mb-3" />,
              ...
              <div key={idx} className="bg-white shadow-md p-8 rounded-xl hover:shadow-lg transition">
                {item.icon}
                <h4 className="font-semibold text-gray-800 text-2xl mb-2">{item.title}</h4>
